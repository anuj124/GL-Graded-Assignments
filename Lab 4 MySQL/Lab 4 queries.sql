...
select cus_name, cus_gender from customer where cus_name like 'A%' or '%A';

select supplier.* from supplier where supp_id in (select supp_id from supplier_pricing group by supp_id having count(supp_id)>1);
